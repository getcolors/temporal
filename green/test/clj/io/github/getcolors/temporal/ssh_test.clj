...
(defn- with-home
  ...
  [f]
  (let [home (str (fs/create-temp-dir {:prefix "temporal-ssh"}))]
    (try
  ...
  (with-home
    (fn [home]
      (let [opts (ssh/with-machine-key (assoc (keygen) :green/event :create))]
        (is (= (str (io/file home ".ssh" "temporal-keygen-fixture")) (:ssh-private-key-path opts)))
        (is (= (str (io/file home ".ssh" "temporal-keygen-fixture.pub")) (:ssh-public-key-path opts)))))))
...
(deftest identity-args-select-the-generated-key-only-in-keygen-mode
  ;; The acceptance verb's ssh threads the private key path in keygen mode:
  ;; nothing guarantees an agent holds it.
  (with-home
    (fn [home]
      (let [opts (ssh/with-machine-key (assoc (keygen) :green/event :create))]
  ...
  (with-home
    (fn [home]
      (let [opts (ssh/ensure-key! (assoc (keygen) :green/event :create) (constantly nil))
            prv (io/file home ".ssh" "temporal-keygen-fixture")
            pub (io/file home ".ssh" "temporal-keygen-fixture.pub")]
        ...
        (testing "ed25519, no passphrase, profile-named comment"
          ...
          (is (str/includes? (slurp pub) "temporal-keygen-fixture managed by Colors")))
...
(deftest a-key-without-state-is-never-overwritten
  (with-home
    (fn [home]
      (let [prv (str (io/file home ".ssh" "temporal-keygen-fixture"))]
...
(deftest preflight-refuses-a-foreign-key-and-says-do-not-delete-it
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "temporal-keygen-fixture.pub")) "ssh-ed25519 OURS comment")
      (with-redefs [once-ssh/fetch-account-keys
                    (fn [_ _] [{:id "abc" :name "temporal-keygen-fixture" :public "ssh-ed25519 THEIRS"}])]
...
(deftest delete-removes-the-keypair
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "temporal-keygen-fixture")) "private")
      (write! (str (io/file home ".ssh" "temporal-keygen-fixture.pub")) "public")
      (ssh/cleanup-step (assoc (keygen) :green/event :delete :ssh-keygen true))
      (is (not (.exists (io/file home ".ssh" "temporal-keygen-fixture"))))
      (is (not (.exists (io/file home ".ssh" "temporal-keygen-fixture.pub"))))
...
(deftest cleanup-is-inert-on-create-and-in-opt-out-mode
  (with-home
    (fn [home]
      ...
      (is (.exists (io/file home ".ssh" "temporal-keygen-fixture")))
