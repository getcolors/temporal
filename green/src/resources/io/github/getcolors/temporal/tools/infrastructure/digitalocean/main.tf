...
<% endif %># Looking up by region returns that region's existing default VPC. The
...
<% endif %>  backups  = <{ digitalocean-backups }>
...
output "params" {
  value = { provider = "digitalocean", ip = digitalocean_droplet.temporal.ipv4_address, user = "root", sudoer = "root", name = "<{ compute-name }>"<% if ssh-keygen %>, ssh_key_id = digitalocean_ssh_key.machine.id<% endif %> }
}
