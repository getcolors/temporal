(ns io.github.getcolors.temporal.compute
...
(defn requirements [opts]
  (let [ssh (request/source-cidrs opts "ssh-sources" "temporal-ssh-sources") http (request/source-cidrs opts "http-sources" "temporal-http-sources")]
    {:single_host true :legacy_state_keys [(str (:profile opts) "/temporal-infrastructure.tfstate")]
...
(defn attach [opts result]
  (cond
    ...
    (= "destroyed" (:status result)) (assoc opts :green/exit 0 :temporal/already-destroyed true)
