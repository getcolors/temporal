...
# the SSH Keypair Standard has two modes and parity means both keygen and
# opt-out hold in every colour. Both render under the local state backend.
#
...
build_variant() {
  ...
  (cd "$root/green" && TEMPORAL_LIB_ROOT="$root" ./green build -f "$tmp/$variant-green.yml" >/dev/null)
  (cd "$root/red" && TEMPORAL_LIB_ROOT="$root/red" ./red build -f "$tmp/$variant-red.yml" >/dev/null)
  (cd "$root/blue" && uv run python -m package_temporal_blue build -f "$tmp/$variant-blue.yml" >/dev/null)
