...
def requirements(opts):
    ssh = source_cidrs(opts, 'ssh-sources', 'temporal-ssh-sources')
    http = source_cidrs(opts, 'http-sources', 'temporal-http-sources')
    return {'single_host': True, 'legacy_state_keys': [opts['profile'] + '/temporal-infrastructure.tfstate'],
...
def attach(opts, result):
    ...
    if result['status'] == 'destroyed':
        return {**opts, 'blue/exit': 0, 'temporal/already-destroyed': True}
    params = node(result)
