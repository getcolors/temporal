...
io.github.getcolors.temporal.validate. The compute and DNS credential maps are
this package's own; the state-backend registry is ONCE's.
...
forbidden_vpc_keys = ["digitalocean-vpc-id", "digitalocean-vpc-uuid",
...
def state_errors(opts: dict) -> list[str]:
    ...
    if not _is_bool(opts.get("digitalocean-backups")):
    ...
    if opts.get("digitalocean-region") != "ams3":
        errors.append(":digitalocean-region must be the configured Amsterdam region ams3")
...
def secret_errors(opts: dict) -> list[str]:
    keys = ["do-token", "cloudflare-api-token", *_backend_entry(opts).get("secrets", [])]
