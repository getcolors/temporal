...
from package_temporal_blue import ssh
...
def test_a_dry_run_renders_the_placeholder_and_real_events_the_real_path(home):
    dry = ssh.with_machine_key({**keygen(), "blue/event": "create", "blue/dry-run": True})
    assert dry["ssh-public-key-path"].startswith(ssh.build_placeholder_dir)
    real = ssh.with_machine_key({**keygen(), "blue/event": "create"})
    assert real["ssh-private-key-path"] == str(home / ".ssh" / "temporal-keygen-fixture")
    assert real["ssh-public-key-path"] == str(home / ".ssh" / "temporal-keygen-fixture.pub")
...
def test_identity_args_select_the_generated_key_only_in_keygen_mode(home):
    ...
    assert ssh.private_key_path(opts) == str(home / ".ssh" / "temporal-keygen-fixture")
...
async def test_first_create_generates_the_keypair(home):
    ...
    prv = home / ".ssh" / "temporal-keygen-fixture"
    pub = home / ".ssh" / "temporal-keygen-fixture.pub"
    assert "blue/err" not in opts, opts.get("blue/err")
    assert prv.exists()
    assert pub.exists()
    assert "ssh-ed25519" in pub.read_text()
    assert "temporal-keygen-fixture managed by Colors" in pub.read_text()
...
async def test_a_key_without_state_is_never_overwritten(home):
    ...
    write(home / ".ssh" / "temporal-keygen-fixture.pub", "ssh-ed25519 AAAA test")
    opts = await ssh.ensure_key({**keygen(), "blue/event": "create"}, none_state)
...
def test_preflight_refuses_a_foreign_key_and_says_do_not_delete_it(home):
    write(home / ".ssh" / "temporal-keygen-fixture.pub", "ssh-ed25519 OURS comment")
    opts = ssh.preflight(
        ssh.with_machine_key({**keygen(), "blue/event": "create"}),
        lambda _p, _t: [{"id": "abc", "name": "temporal-keygen-fixture",
...
def test_delete_removes_the_keypair_and_cleanup_is_otherwise_inert(home):
    write(home / ".ssh" / "temporal-keygen-fixture", "private")
    write(home / ".ssh" / "temporal-keygen-fixture.pub", "public")
    ssh.cleanup_step({**keygen(), "blue/event": "create", "ssh-keygen": True})
    assert (home / ".ssh" / "temporal-keygen-fixture").exists()
    ssh.cleanup_step({**fixture(), "blue/event": "delete"})
    assert (home / ".ssh" / "temporal-keygen-fixture").exists()
    ssh.cleanup_step({**keygen(), "blue/event": "delete", "ssh-keygen": True})
    assert not (home / ".ssh" / "temporal-keygen-fixture").exists()
    assert not (home / ".ssh" / "temporal-keygen-fixture.pub").exists()
